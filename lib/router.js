...
Router.map( function () {
  ...
  this.route('paymentsPlace', {
    path: '/payements/:_id',
    onBeforeAction: function() {
      Meteor.subscribe('Payments', this.params._id);
      this.next();
    },
    data: function() {
      return Places.findOne({_id: this.params._id});
  ...
  this.route('createOrder', {
    ...
    onBeforeAction: function() {
      ...
      if (Session.get('orderId')) {
        Meteor.subscribe('Lines', Session.get('orderId'));
      }
      this.next();
    },
    data: function() {
  ...
  this.route('editOrder', {
    ...
    onBeforeAction: function() {
      ...
      Meteor.subscribe('Sets', this.params.place_id);
      Meteor.subscribe('Orders', this.params.place_id);
      Meteor.subscribe('Lines', this.params.order_id);
      Meteor.subscribe('Options', this.params.place_id);
      this.next();
    },
    data: function() {
  ...
  this.route('adminPlaces', {
    path:'/places',
    template: 'adminPlaces',
    onBeforeAction: function() {
      if (Meteor.loggingIn()) {
        this.render(this.loadingTemplate);
      } else if(!Roles.userIsInRole(Meteor.user(), ['admin'])) {
